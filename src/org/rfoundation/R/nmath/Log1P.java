...
public class Log1P {
    ...
    private static final int nlnrel = 22;
	private static final double xmin = -0.999999985;
	...
	public static double log1p(double x) {
		...
		if(Math.abs(x) < .5 * Rmath.DBL_EPSILON)
		    return x;

		if( (0 < x && x < 1e-8) || (-1e-9 < x && x < 0))
		    return x * (1 - .5 * x);
		/* else */
		return x * (1 - x * Chebyshev.chebyshev_eval(x / .375, alnrcs, nlnrel));
	    }
	    /* else */
	    if (x < xmin) {
	    	/* answer less than half precision because x too near -1 */
	    	// FIXME ML_ERROR(ME_PRECISION, "log1p");
	    }
	    return Math.log(1 + x);
	}
	...
	public static double hypot(double a, double b) {
	    ...
	    if(Double.isNaN(a) || Double.isNaN(b)) /* propagate Na(N)s: */
	        return Double.NaN;
		...
		for(;;) {
		    ...
		    if(Math.abs(r) < 2* Rmath.DBL_EPSILON) break;
		    s = r / t;
		    u = 1. + 2. * s;
		    p *= u ;
