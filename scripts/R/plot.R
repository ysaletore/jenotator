...
for(s in 1:n_samples) {
	...
	while(n <= N-1) {
		...
		if(plotFlanking5) {
			...
			plot(rollmean(flanking5[[s]][,c_flanking5], ROLLMEAN), type='l', col=row_col, lwd=row_lwd, xaxt='n', ylim=ylim, xaxs='r', yaxs='r', ylab='% Peaks');
			title(paste("-1KB"));
		...
		if(plotUTR5) {
			...
			x = x + plot_width * WIDTH;
			if(plotFlanking5) {
				plot(rollmean(utr5[[s]][,c_utr5], ROLLMEAN), type='l', lty=row_lty, col=row_col, lwd=row_lwd, xaxt='n', yaxt='n', ylim=ylim, xaxs='r', yaxs='r', ylab='% Peaks');
			} else {
				plot(rollmean(utr5[[s]][,c_utr5], ROLLMEAN), type='l', lty=row_lty, col=row_col, lwd=row_lwd, xaxt='n', ylim=ylim, xaxs='r', yaxs='r', ylab='% Peaks');
			}
			title(paste("5' UTR"));
			c_utr5 = c_utr5 + 1;
		}
		...
		while(i <= n) {
			if(plotCDS) {
				par(fig=c(x, x+plot_width*WIDTH, y+plot_margin, y+plot_height*HEIGHT-plot_margin), new = TRUE);
				x = x + plot_width * WIDTH;
				plot(rollmean(cds[[s]][,c_cds], ROLLMEAN), type='l', lty=row_lty, col=row_col, lwd=row_lwd, xaxt='n', yaxt='n', ylim=ylim, xaxs='r', yaxs='r');
			...
			if(plotIntrons) {
				par(fig=c(x, x+plot_width*WIDTH, y+plot_margin, y+plot_height*HEIGHT-plot_margin), new = TRUE);
				x = x + plot_width * WIDTH;
				plot(rollmean(introns[[s]][,c_introns], ROLLMEAN), type='l', lty=row_lty, col=row_col, lwd=row_lwd, xaxt='n', yaxt='n', ylim=ylim, xaxs='r', yaxs='r');
		...
		if(plotUTR3) {
			par(fig=c(x, x+plot_width*WIDTH, y+plot_margin, y+plot_height*HEIGHT-plot_margin), new = TRUE);
			x = x + plot_width * WIDTH;
			plot(rollmean(utr3[[s]][,c_utr3], ROLLMEAN), type='l', lty=row_lty, col=row_col, lwd=row_lwd, xaxt='n', yaxt='n', ylim=ylim, xaxs='r', yaxs='r');
			title(paste("3' UTR"));
			c_utr3 = c_utr3 + 1;
		}
		...
		if(plotFlanking3) {
			par(fig=c(x, x+plot_width*WIDTH, y+plot_margin, y+plot_height*HEIGHT-plot_margin), new = TRUE);
			x = x + plot_width * WIDTH;
			plot(rollmean(flanking3[[s]][,c_flanking3], ROLLMEAN), type='l', lty=row_lty, col=row_col, lwd=row_lwd, xaxt='n', yaxt='n', ylim=ylim, xaxs='r', yaxs='r');
			title(paste("+1KB"));
	...
	if(plotFlanking5) {
		par(fig=c(x, x+plot_width*WIDTH, y+plot_margin, y+plot_height*HEIGHT-plot_margin), new = TRUE);
		x = x + plot_width * WIDTH;
		plot(rollmean(flanking5[[s]][,c_flanking5], ROLLMEAN), type='l', lty=row_lty, col=row_col, lwd=row_lwd, xaxt='n', ylim=ylim, xaxs='r', yaxs='r', ylab='% Peaks');
		title(paste("-1KB"));
		c_flanking5 = c_flanking5 + 1;
	}
	...
	if(plotUTR5) {
		...
		if(plotFlanking5) {
			plot(rollmean(utr5[[s]][,c_utr5], ROLLMEAN), type='l', lty=row_lty, col=row_col, lwd=row_lwd, xaxt='n', yaxt='n', ylim=ylim, xaxs='r', yaxs='r');
		} else {
			plot(rollmean(utr5[[s]][,c_utr5], ROLLMEAN), type='l', lty=row_lty, col=row_col, lwd=row_lwd, xaxt='n', ylim=ylim, xaxs='r', yaxs='r', ylab='% Peaks');
		}
...
if(!is.null(options$legend)) {
	x = xmargin + row_xmargin;
	y = ymargin / 4;
	par(fig=c(xmargin/4,1-xmargin/4, y, y + ymargin * 3 / 4), new = TRUE);
	plot.new();
	mylegend = unlist(strsplit(options$legend, ","));
	legend('center', mylegend, col=row_cols, lty=row_ltys, lwd=row_lwds, ncol=2);
}
dev.off();
